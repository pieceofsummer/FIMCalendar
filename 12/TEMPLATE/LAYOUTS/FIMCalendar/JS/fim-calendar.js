// Finds all date/time controls and attaches the jQuery control to them
function SetupDatePickers() {
    $("input[type='text'][id$='_internalDateTimeEditor']").datetimepicker({
        changeMonth: true,
        changeYear: true,
...
function GetUserLanguage() {
    var localeMap = {
        1: 'ar', 2: 'bg', 3: 'ca', 4: 'zh-CHS', 5: 'cs', 6: 'da', 7: 'de', 8: 'el', 9: 'en',
    ...
    var locale = ((lcid in localeMap) ? localeMap[lcid] : browserLanguage).toLowerCase();
...
$(document).ready(function () {
    ...
    //if (uoc.length == 1) {
    //    if (uoc[0].addEventListener) {
    //        uoc[0].addEventListener("propertychange",
    //            function (e) {
    //                if (e.propertyName == "asyncStatus" && e.srcElement.asyncStatus == "done")
    //                    SetupDatePickers();
    //            });
    //    } else if (uoc[0].attachEvent) {
    //        uoc[0].attachEvent("onpropertychange",
    //            function (e) {
    //                if (e.propertyName == "asyncStatus" && e.srcElement.asyncStatus == "done")
    //                    SetupDatePickers();
    //            });
...
function pageLoad(sender, args) {
    ...
    // Call SetupDatePickers after an update panel refresh in order to find new controls that have appeared
